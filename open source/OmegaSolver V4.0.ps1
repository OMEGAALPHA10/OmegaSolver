...
# ==============================================================================
# PROYECTO: OmegaSolver V4.0 (Beta Final - Manual Adaptativo + Carga Inicial)
# ===============================================================================
...
if (-not (Test-OmegaAdministrator)) {
    try {
        ...
        if (-not $scriptPath) {
            [System.Windows.MessageBox]::Show("Guarda el script como archivo .ps1 y ejecÃºtalo de nuevo.", "OmegaSolver V4.0", 'OK', 'Warning') | Out-Null
            exit 1
        }
        $hostExe = $null
        ...
        [System.Windows.MessageBox]::Show("OmegaSolver necesita permisos de administrador para ejecutarse.", "OmegaSolver V4.0", 'OK', 'Warning') | Out-Null
        exit 1
    }
}
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        ...
        Title="OmegaSolver V4.0" Height="720" Width="1080"
        ...
        <Grid Margin="12">
            ...
            <Grid Grid.Row="0" Margin="0,0,0,10">
                ...
                <StackPanel Grid.Column="0">
                    <TextBlock Text="OmegaSolver V4.0" FontSize="22" FontWeight="Bold" Foreground="{DynamicResource ThemeAccent}"/>
                ...
                <StackPanel Grid.Column="1" Orientation="Horizontal" VerticalAlignment="Top">
                    ...
                    <ComboBox x:Name="cmbTheme" Width="150" Height="28" Margin="0,0,12,0" FontSize="12"/>
...
function Show-OmegaManual {
    $currentTheme = $global:cmbTheme.SelectedItem
    ...
    if ($isAdvanced) {
        $esItems = @(
            ...
            'H|ðŸŽ¨  TEMAS Y MODOS',
            'I|17 Paletas|Colores definidos con DynamicResource para retematizaciÃ³n en vivo.',
        ...
        $enItems = @(
            ...
            'H|ðŸŽ¨  THEMES &amp; MODES',
            'I|17 Palettes|Colors defined via DynamicResource for live rethematization.',
        ...
        $esItems = @(
            ...
            'H|ðŸŽ¨  TEMAS Y MODOS',
            'I|Temas visuales|Elige entre 17 estilos de color distintos para la interfaz.',
        ...
        $enItems = @(
            ...
            'H|ðŸŽ¨  THEMES &amp; MODES',
            'I|Visual Themes|Choose from 17 different color styles for the interface.',
                ...
                <Grid>
                    <StackPanel Orientation="Horizontal" VerticalAlignment="Center" Margin="35,0,0,0">
                        ...
                        <StackPanel VerticalAlignment="Center">
                            ...
                            <TextBlock x:Name="lblManualMode" Text="OmegaSolver V4.0" FontSize="13" Foreground="White" Opacity="0.85"/>
                        </StackPanel>
                    </StackPanel>
...
"@
    ...
    $panelBg  = $bc.ConvertFromString($palette.PanelBg)
    $cardBg   = $bc.ConvertFromString($palette.CardBg)
    $accent   = $bc.ConvertFromString($palette.Accent)
    $mainTxt  = $bc.ConvertFromString($palette.MainText)
    $mutedTxt = $bc.ConvertFromString($palette.MutedText)
    $border   = $bc.ConvertFromString($palette.Border)

    $mw.Resources["ManualPanelBg"] = $panelBg
    $mw.Resources["ManualCardBg"]  = $cardBg
    $mw.Resources["ManualAccent"]  = $accent
    $mw.Resources["ManualText"]    = $mainTxt
    $mw.Resources["ManualMuted"]   = $mutedTxt
    $mw.Resources["ManualBorder"]  = $border

    if ($isAdvanced) {
        $mw.FindName("lblManualMode").Text = "OmegaSolver V4.0 Â· Advanced Mode / Modo Avanzado"
        $mw.FindName("lblModeBadge").Text  = "ðŸ”§  Mostrando documentaciÃ³n tÃ©cnica detallada Â· Showing detailed technical documentation"
    } else {
        $mw.FindName("lblManualMode").Text = "OmegaSolver V4.0 Â· Basic Mode / Modo BÃ¡sico"
    ...
    Add-ManualContent -StackPanel $mw.FindName("stackEs") -Items $esItems -AccentB $accent -TextB $mainTxt -MutedB $mutedTxt
    Add-ManualContent -StackPanel $mw.FindName("stackEn") -Items $enItems -AccentB $accent -TextB $mainTxt -MutedB $mutedTxt
...
function Set-OmegaThemeResources {
    ...
    foreach ($resKey in $map.Keys) {
        ...
        try {
            $brush = $bc.ConvertFromString($hexStr)
...
$script:OmegaThemes = @{
    ...
    "Windhawk Glass" = @{
        WindowBg="#0A0612"; PanelBg="#B3120A22"; CardBg="#A61F0F2E"; InputBg="#99100514"
        MainText="#F8F4FF"; SecondaryText="#DDD1F0"; MutedText="#9B8CC2"; LabelText="#B8A6DD"
        Accent="#FF6B35"; AccentAlt="#C77DFF"; Border="#664A3A7F"; BorderStrong="#99A855F7"
        ButtonBg="#992A1A47"; ButtonHover="#B3402857"; ButtonPressed="#801F1236"
        BorderHover="#FF6B35"; BorderPressed="#C77DFF"
        PrimaryBg="#FF6B35"; PrimaryHover="#E85A28"; PrimaryPressed="#C94A1F"
        RevertBg="#992A0A0A"; RevertBorder="#FF6B6B"; RevertHover="#B3401010"; RevertPressed="#801F0505"
        ManualBg="#993D2A5F"; ManualHover="#B3553D7D"; ManualPressed="#802A1A47"; Log="#FFB088"
    }
}
...
function Set-OmegaModeUI {
    ...
    try {
        if ($Advanced) {
            ...
            if ($window.WindowState -eq [System.Windows.WindowState]::Normal) { $window.Height = 900 }
            ...
            $global:chkAdvanced.Content = "ðŸ”§ Modo Avanzado"
        } else {
            $global:statsPanel.Visibility = 'Collapsed'; $global:logPanel.Visibility = 'Collapsed'
            if ($window.WindowState -eq [System.Windows.WindowState]::Normal) { $window.Height = 720 }
...
if ($themeList -contains $savedTheme) {
    ...
    Set-OmegaThemeResources -Palette $script:OmegaThemes["Spotify Neon"]
}

Set-OmegaModeUI -Advanced $false
Refresh-OmegaRepairDrives
Write-OmegaLog "OmegaSolver V4.0 Beta iniciado."
            ...
            <Border Grid.Row="0" Background="{DynamicResource ThemeAccent}" CornerRadius="11,11,0,0">
                <StackPanel Orientation="Horizontal" VerticalAlignment="Center" Margin="35,0,0,0">
                    ...
                    <StackPanel VerticalAlignment="Center">
                        ...
                        <TextBlock Text="OmegaSolver V4.0" FontSize="13" Foreground="White" Opacity="0.85"/>
                    </StackPanel>
                </StackPanel>
            </Border>
...
function Invoke-OmegaTask {
    ...
    $bc = New-Object System.Windows.Media.BrushConverter
    $lw.Resources["ThemePanelBackground"]  = $bc.ConvertFromString($palette.PanelBg)
    $lw.Resources["ThemeAccent"]           = $bc.ConvertFromString($palette.Accent)
    $lw.Resources["ThemeMainText"]         = $bc.ConvertFromString($palette.MainText)
    $lw.Resources["ThemeInputBackground"]  = $bc.ConvertFromString($palette.InputBg)
    $lw.Resources["ThemeMutedText"]        = $bc.ConvertFromString($palette.MutedText)
    $lw.Resources["ThemeCardBackground"]   = $bc.ConvertFromString($palette.CardBg)
...
# ---------- AUTO-DIAGNÃ“STICO AL INICIAR (CON PANTALLA DE CARGA) ----------
$window.Add_Loaded({
